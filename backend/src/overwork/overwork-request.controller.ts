...
export class OverworkRequestController {
  ...
  // Admin workload analysis endpoint - for pegawai management page
  @Get('admin/workload/analysis')
  async getAdminWorkloadAnalysis(
    @Query('month') month?: string,
    @Query('year') year?: string
  ) {
    try {
      ...
      // Transform data for admin analysis view
      const analysisData = await Promise.all(workloadData.map(async user => {
        const workload = user.workloadSummary;
        
        // Calculate weekly shifts for this user
        const weeklyShifts = await this.validationService.getUserWeeklyShifts(user.userId);
        
        // Calculate daily shifts for this user
        const dailyShifts = await this.validationService.getUserDailyShifts(user.userId);
        
        // Calculate utilization rate
        const utilizationRate = Math.round((workload.currentShifts / workload.maxShifts) * 100);
        
        // Determine status
        let status: 'NORMAL' | 'WARNING' | 'CRITICAL' = 'NORMAL';
        if (workload.status === 'OVERWORKED' || utilizationRate > 90) {
          status = 'CRITICAL';
        } else if (workload.status === 'APPROACHING_LIMIT' || utilizationRate > 70) {
          status = 'WARNING';
        }
        ...
        return {
          ...
          namaBelakang: user.namaBelakang,
          employeeId: user.employeeId,
          currentShifts: workload.currentShifts,
          maxShifts: workload.maxShifts,
          weeklyHours: workload.totalHours, // Frontend expects weeklyHours
          weeklyShifts: weeklyShifts,
          dailyShifts: dailyShifts,
          status: status,
          utilizationRate: utilizationRate,
          canTakeMoreShifts: workload.canTakeMoreShifts,
          isDisabledForShifts: workload.isDisabledForShifts,
          overworkRequestRequired: workload.overworkRequestRequired
        };
      }));

      return analysisData;
    } catch (error) {
      throw new Error(error.message || 'Failed to get admin workload analysis');
    }
  }
